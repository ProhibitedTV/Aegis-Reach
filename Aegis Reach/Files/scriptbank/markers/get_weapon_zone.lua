...
-- Get Weapon Zone v6
-- DESCRIPTION: Gets unarmed melee mode or closest weapon within set range to this zone and gives to the player, then destroys the zone.
-- DESCRIPTION: [@WeaponMode=1(1=Unarmed Melee, 2=Closest Weapon)]
...
function get_weapon_zone_properties(e, WeaponMode, SearchRange, ZoneHeight, SpawnAtStart, WeaponAmmunition, WeaponState)
    gwzone[e] = g_Entity[e]
	gwzone[e].WeaponMode = WeaponMode or 1	
...
function get_weapon_zone_main(e)
	...
	if status[e] == "init" then
		...
		if gwzone[e].WeaponMode	== 1 then
			gwzweapon[e] = GetWeaponID("enhanced\\gloves_unarmed")
		end
		if gwzone[e].WeaponMode	== 2 then 	
		...
		if g_Entity[e]['plrinzone'] == 1 and g_PlayerPosY > g_Entity[e]['y'] and g_PlayerPosY < g_Entity[e]['y'] + gwzone[e].ZoneHeight then
			if doonce[e] == 0 then
				if gwzone[e].WeaponMode	== 1 then
					...
					SetWeaponSlot(0,gwzweapon[e],0)
					ChangePlayerWeaponID(gwzweapon[e])
				...
				if gwzone[e].WeaponMode	== 2 then
					...
					if gwzweapon[e] == 0 then PromptLocal(e,"No weapon found within range") end
					if gwzweapon[e] ~= 0 then wait[e] = g_Time + 500 end
					doonce[e] = 1
				end	
				if played[e] == 0 then 
