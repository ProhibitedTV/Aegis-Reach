...
local function rect(x,y,w,h,r,g,b,a)
 if not pixel and LoadImage and CreateSprite then
  local ok,id=pcall(function() return CreateSprite(LoadImage('scriptbank\\aegis_reach\\images\\hud_pixel.png')) end)
...
function fl_hud_draw(objective,meters,bearing,yaw,shield,armour)
 ...
 rect(38,2.1,24,4.7,8,16,22,110);rect(39.2,5.7,21.6,.13,153,179,188,180);local cardinals={[0]='N',[45]='NE',[90]='E',[135]='SE',[180]='S',[225]='SW',[270]='W',[315]='NW'}
 for angle=0,345,15 do local d=fl_hud_delta(angle,yaw);if math.abs(d)<=60 then local x=50+d*.18;rect(x,cardinals[angle] and 4.7 or 5.2,.10,cardinals[angle] and 1 or .5,179,203,212,230);if cardinals[angle] then TextCenterOnXColor(x,2.5,1,cardinals[angle],210,225,230) end end end
 rect(49.9,5.5,.2,1.1,230,240,240,255);local target_x=50+math.max(-60,math.min(60,delta))*.18;rect(target_x-.18,6.1,.36,.65,244,196,100,255)
 rect(83,86.8,15,11.2,8,16,22,185);TextColor(84,87.8,1,'SHIELD  '..math.floor(shield),116,211,224);meter(91,shield,65,183,236);TextColor(84,92.4,1,'ARMOUR  '..math.floor(armour),232,239,239);meter(95.6,armour,212,222,225)
end
