...
local profiles={ARRIVAL={seconds=9.55,fade=.35,fls=58,fle=76},ARRIVAL_HANDOFF={seconds=15.9,fade=.35,fls=64,fle=84},MIRA_SIGNAL={seconds=9.95,fade=.35,fls=70,fle=84},AEGIS_REVEAL={seconds=10.75,fade=.35,fls=58,fle=90},EXTRACTION={seconds=7.75,fade=.35,fls=62,fle=82}}
...
local function update_story_timeline(beat,elapsed)
 if beat=="ARRIVAL" then
  if elapsed>=350 then mark_line("FL01_KES_001","FL01_KES_001","KESTREL: Seven, Meridian Control missed two scheduled check-ins.","",3.6) end
  if elapsed>=4300 then mark_line("FL01_KES_002","FL01_KES_002","KESTREL: Forty-two evacuees never reached orbit. Mira Sen is still missing.","",4.6) end
 elseif beat=="ARRIVAL_HANDOFF" then
  if elapsed>=350 then mark_line("FL01_KES_003","FL01_KES_003","KESTREL: Mira's last burst: 'Northstar offline. Array waking.' Then a Warden transponder crossed Gate Zero-Seven.","",6.2) end
  if elapsed>=6900 then mark_line("FL01_KES_004","FL01_KES_004","KESTREL: Restore Northstar. Recover the manifest. Find our people.","",4.0) end
  if elapsed>=11250 then mark_line("FL01_KES_005","FL01_KES_005","KESTREL: You are down. I will hold above the shelf until you call.","",4.0) end
