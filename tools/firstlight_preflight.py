...
def main():
 ...
 report={'map':str(MAP.relative_to(ROOT)),'map_sha256':sha256(MAP),'map_bytes':MAP.stat().st_size,'map_version':version,'entity_count':len(entities),'unsafe_weapon_pickups':bad,'archive_encrypted':encrypted,'qa_observational':True,'tracks':track_info,'tests':tests,'biosphere_entity_count':len(biosphere),'responsive_practical_light_count':len(native_practicals),'combat_cover_count':len(combat_cover),'combat_light_count':len(combat_lights),'authored_enemy_start_count':len(authored_enemies),'clear_lz_center':combat.get('clear_lz_center',False),'cineguru_camera_count':len(cine),'diegetic_camera_rig_count':len(camera_rigs),'kestrel_vehicle_count':len(vehicles),'kestrel_boarding_collision_count':len(boarding_collision),'dialogue_line_count':len(manifest['lines']),'dialogue_audio_bound_count':len(dialogue_audio),'native_playtest_required':True}
 REPORT.write_text(json.dumps(report,indent=2));print('FIRST LIGHT // PREFLIGHT PASS');print('Map entities:',len(entities));print('Map SHA256:',report['map_sha256'][:16]);print('Dialogue:',report['dialogue_line_count'],'lines /',report['dialogue_audio_bound_count'],'voice files bound');print('Kestrel:',len(vehicles),'vehicle entities / visible insertion + extraction /',len(boarding_collision),'landed boarding collision proxy');print('CineGuru story layer:',len(cine),'cameras / 17-cut insertion /',len(camera_rigs),'mixed diegetic camera rig / fail-open');print('Native MAX playtest is still required.')
