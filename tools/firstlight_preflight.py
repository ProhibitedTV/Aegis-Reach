...
CineGuru story cameras, and tactical combat geometry generated by the AAA layer.
"""
...
def main():
    ...
    tests = [
        ...
        'test_firstlight_biosphere.py',
        'test_firstlight_cinematics.py',
    ]
    for name in tests:
        run_test(name)

    combat,layout,combat_cover,combat_lights,authored_enemies=load_combat_geometry_manifest()
    cine_cameras=[p for p in layout if p.get('kind')=='cinematic_camera']
    cine_controllers=[p for p in layout if p.get('name')=='FIRST LIGHT // CINEMATIC']
    if len(cine_cameras)!=4 or len(cine_controllers)!=1:
        raise SystemExit(
            'FIRST LIGHT // PREFLIGHT FAILED: expected four CineGuru story cameras and one cinematic coordinator; rebuild the authored map'
        )
    ...
    required_scripts = [
        ...
        'firstlight_cinematic.lua',
        'firstlight_biosphere.lua',
    ]
    ...
    cineguru_scripts = ['cg_cinematic_camera.lua','cg_lib.lua','cg_pnoise.lua']
    for name in cineguru_scripts:
        path=FILES/'scriptbank/Cine Guru MAX'/name
        if not path.is_file():
            raise SystemExit(f'FIRST LIGHT // PREFLIGHT FAILED: missing CineGuru dependency {name}')
    ...
    if 'music_cinematic_duck' not in score_text:
        raise SystemExit('FIRST LIGHT // PREFLIGHT FAILED: score controller is not wired for CineGuru ducking')

    biosphere=[p for p in layout if str(p.get('kind','')).startswith('biosphere_')]
    if len(biosphere)<30:
        raise SystemExit('FIRST LIGHT // PREFLIGHT FAILED: primitive biosphere layer is incomplete')
    ...
    report = {
        ...
        'biosphere_gate': True,
        'biosphere_entity_count':len(biosphere),
        ...
        'cineguru_camera_count': len(cine_cameras),
        'cineguru_controller_count': len(cine_controllers),
        'cineguru_fail_open': True,
    ...
    print('Primitive biosphere:',len(biosphere),'ambient entities / visual-only')
    print('Combat geometry:',len(combat_cover),'cover /',len(combat_lights),'backlights /',len(authored_enemies),'authored starts')
    print('CineGuru story layer:',len(cine_cameras),'cameras /',len(cine_controllers),'coordinator / fail-open')
