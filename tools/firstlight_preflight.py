...
def main():
 ...
 cine=[p for p in layout if p.get('kind')=='cinematic_camera'];controllers=[p for p in layout if p.get('name')=='FIRST LIGHT // CINEMATIC'];camera_rigs=[p for p in layout if p.get('name')=='FIRST LIGHT // CAMERA RIG'];native_chains=[p for p in layout if p.get('name')=='FIRST LIGHT // CINEGURU NATIVE CHAIN'];opening_directors=[p for p in layout if p.get('name')=='FIRST LIGHT // OPENING DIRECTOR'];title_layer=[p for p in layout if str(p.get('kind','')).startswith('title_')];vehicles=[p for p in layout if p.get('kind')=='vehicle'];dialogue_audio=[p for p in layout if p.get('kind')=='dialogue_audio'];boarding_collision=[p for p in layout if p.get('name')=='FL KESTREL BOARDING COLLISION']
 if len(cine)!=20 or len(controllers)!=1:raise SystemExit('FIRST LIGHT // PREFLIGHT FAILED: expected twenty authored camera markers and one story coordinator')
 if len(camera_rigs)!=1:raise SystemExit('FIRST LIGHT // PREFLIGHT FAILED: expected one diegetic Kestrel camera rig placement')
 if len(native_chains)!=1:raise SystemExit('FIRST LIGHT // PREFLIGHT FAILED: expected one native CineGuru graph controller')
 if len(opening_directors)!=1:raise SystemExit('FIRST LIGHT // PREFLIGHT FAILED: expected one opening compatibility marker')
 if len(title_layer)<13:raise SystemExit('FIRST LIGHT // PREFLIGHT FAILED: title-screen convergence skyline incomplete')
 ...
 native_chain_ctrl=[e for e in entities if str(e.get('101:eleprof.name_s',''))=='FIRST LIGHT // CINEGURU NATIVE CHAIN']
 if len(native_chain_ctrl)!=1 or normalized_script(native_chain_ctrl[0])!='aegis_reach\\firstlight_cineguru_chain.lua':raise SystemExit('FIRST LIGHT // PREFLIGHT FAILED: native CineGuru graph controller is missing or bound to the wrong script')
 native_opening_directors=[e for e in entities if str(e.get('101:eleprof.name_s',''))=='FIRST LIGHT // OPENING DIRECTOR']
 if len(native_opening_directors)!=1 or normalized_script(native_opening_directors[0])!='aegis_reach\\firstlight_opening_director.lua':raise SystemExit('FIRST LIGHT // PREFLIGHT FAILED: opening compatibility marker is missing or bound to the wrong script')
 ...
 report={'map':str(MAP.relative_to(ROOT)),'map_sha256':sha256(MAP),'map_bytes':MAP.stat().st_size,'map_version':version,'entity_count':len(entities),'unsafe_weapon_pickups':bad,'archive_encrypted':encrypted,'qa_observational':True,'tracks':track_info,'tests':tests,'biosphere_entity_count':len(biosphere),'responsive_practical_light_count':len(native_practicals),'combat_cover_count':len(combat_cover),'combat_light_count':len(combat_lights),'authored_enemy_start_count':len(authored_enemies),'clear_lz_center':combat.get('clear_lz_center',False),'cineguru_camera_count':len(cine),'diegetic_camera_rig_count':len(camera_rigs),'native_cineguru_chain_count':len(native_chains),'opening_compatibility_marker_count':len(opening_directors),'opening_camera_owner':'mission-hud-native','title_screen_convergence_count':len(title_layer),'kestrel_vehicle_count':len(vehicles),'kestrel_boarding_collision_count':len(boarding_collision),'dialogue_line_count':len(manifest['lines']),'dialogue_audio_bound_count':len(dialogue_audio),'native_playtest_required':True}
 REPORT.write_text(json.dumps(report,indent=2));print('FIRST LIGHT // PREFLIGHT PASS');print('Map entities:',len(entities));print('Map SHA256:',report['map_sha256'][:16]);print('Dialogue:',report['dialogue_line_count'],'lines /',report['dialogue_audio_bound_count'],'voice files bound');print('Kestrel:',len(vehicles),'vehicle entities / visible insertion + extraction /',len(boarding_collision),'landed boarding collision proxy');print('Opening: mission HUD-native camera owner / 17 hard cuts /',len(cine),'authored markers / CineGuru only for later story beats');print('Title-screen convergence:',len(title_layer),'visual-only skyline entities');print('Native MAX playtest is still required.')
