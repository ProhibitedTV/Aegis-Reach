...
def main():
 ...
 if len(opening_directors)!=1:raise SystemExit('FIRST LIGHT // PREFLIGHT FAILED: expected one deterministic opening director')
 ...
 if len(native_opening_directors)!=1 or normalized_script(native_opening_directors[0])!='aegis_reach\\firstlight_opening_director.lua':raise SystemExit('FIRST LIGHT // PREFLIGHT FAILED: deterministic opening director is missing or bound to the wrong script')
 ...
 report={'map':str(MAP.relative_to(ROOT)),'map_sha256':sha256(MAP),'map_bytes':MAP.stat().st_size,'map_version':version,'entity_count':len(entities),'unsafe_weapon_pickups':bad,'archive_encrypted':encrypted,'qa_observational':True,'tracks':track_info,'tests':tests,'biosphere_entity_count':len(biosphere),'responsive_practical_light_count':len(native_practicals),'combat_cover_count':len(combat_cover),'combat_light_count':len(combat_lights),'authored_enemy_start_count':len(authored_enemies),'clear_lz_center':combat.get('clear_lz_center',False),'cineguru_camera_count':len(cine),'diegetic_camera_rig_count':len(camera_rigs),'native_cineguru_chain_count':len(native_chains),'deterministic_opening_director_count':len(opening_directors),'title_screen_convergence_count':len(title_layer),'kestrel_vehicle_count':len(vehicles),'kestrel_boarding_collision_count':len(boarding_collision),'dialogue_line_count':len(manifest['lines']),'dialogue_audio_bound_count':len(dialogue_audio),'native_playtest_required':True}
 REPORT.write_text(json.dumps(report,indent=2));print('FIRST LIGHT // PREFLIGHT PASS');print('Map entities:',len(entities));print('Map SHA256:',report['map_sha256'][:16]);print('Dialogue:',report['dialogue_line_count'],'lines /',report['dialogue_audio_bound_count'],'voice files bound');print('Kestrel:',len(vehicles),'vehicle entities / visible insertion + extraction /',len(boarding_collision),'landed boarding collision proxy');print('Opening:',len(opening_directors),'deterministic director / 17 hard cuts /',len(cine),'authored camera mounts /',len(camera_rigs),'diegetic mount rig');print('Title-screen convergence:',len(title_layer),'visual-only skyline entities');print('Native MAX playtest is still required.')
