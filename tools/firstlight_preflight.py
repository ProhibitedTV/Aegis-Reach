...
def main():
 ...
 cine=[p for p in layout if p.get('kind')=='cinematic_camera'];controllers=[p for p in layout if p.get('name')=='FIRST LIGHT // CINEMATIC'];camera_rigs=[p for p in layout if p.get('name')=='FIRST LIGHT // CAMERA RIG'];native_chains=[p for p in layout if p.get('name')=='FIRST LIGHT // CINEGURU NATIVE CHAIN'];title_layer=[p for p in layout if str(p.get('kind','')).startswith('title_')];vehicles=[p for p in layout if p.get('kind')=='vehicle'];dialogue_audio=[p for p in layout if p.get('kind')=='dialogue_audio'];boarding_collision=[p for p in layout if p.get('name')=='FL KESTREL BOARDING COLLISION']
 ...
 report={'map':str(MAP.relative_to(ROOT)),'map_sha256':sha256(MAP),'map_bytes':MAP.stat().st_size,'map_version':version,'entity_count':len(entities),'unsafe_weapon_pickups':bad,'archive_encrypted':encrypted,'qa_observational':True,'tracks':track_info,'tests':tests,'biosphere_entity_count':len(biosphere),'responsive_practical_light_count':len(native_practicals),'combat_cover_count':len(combat_cover),'combat_light_count':len(combat_lights),'authored_enemy_start_count':len(authored_enemies),'clear_lz_center':combat.get('clear_lz_center',False),'cineguru_camera_count':len(cine),'diegetic_camera_rig_count':len(camera_rigs),'native_cineguru_chain_count':len(native_chains),'title_screen_convergence_count':len(title_layer),'kestrel_vehicle_count':len(vehicles),'kestrel_boarding_collision_count':len(boarding_collision),'dialogue_line_count':len(manifest['lines']),'dialogue_audio_bound_count':len(dialogue_audio),'native_playtest_required':True}
 REPORT.write_text(json.dumps(report,indent=2));print('FIRST LIGHT // PREFLIGHT PASS');print('Map entities:',len(entities));print('Map SHA256:',report['map_sha256'][:16]);print('Dialogue:',report['dialogue_line_count'],'lines /',report['dialogue_audio_bound_count'],'voice files bound');print('Kestrel:',len(vehicles),'vehicle entities / visible insertion + extraction /',len(boarding_collision),'landed boarding collision proxy');print('CineGuru story layer:',len(cine),'cameras / 17-cut insertion / native linked opener /',len(camera_rigs),'mixed diegetic camera rig / fail-open');print('Title-screen convergence:',len(title_layer),'visual-only skyline entities');print('Native MAX playtest is still required.')
