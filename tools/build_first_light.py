...
# Few geologic props provide local cover; the ridges themselves are native terrain.
...
# Retrofitted checkpoint: cover faces the escape road and the civilian route alike.
...
light_locations=[(-1750,-7000,500,0xE7BB80,720),(-850,-2920,600,0x67D8EA,850),(850,-2920,600,0x67D8EA,850),(-1700,-1200,600,0xFFAC65,950),(-1700,-50,600,0xFFAC65,950),(1100,200,600,0xAACEDD,650),(1600,1150,600,0xAACEDD,650),(-900,2800,600,0x70C7DC,1000),(900,3400,600,0x70C7DC,1000),(-750,-2200,600,0x67D8EA,1000)]
...
for idx,(x,z,y,color,radius) in enumerate(light_locations,1):
 y=ground(x,z)
 fixture_height=265 if 250<x<2300 and -200<z<1650 else 305
 add(LAMP,'Working lamp / '+str(idx),x,z,y=y,scale=85 if fixture_height==265 else 100)
...
for key,val in {'AmbientMusicTrack':'','AmbientMusicTrackVolume':0,'FogNearest#':9500,'FogDistance#':31000,'Exposure':1.40,'SunIntensity':1.55,'BloomStrength':0.15}.items():visual=patch_setting(visual,key,val)
